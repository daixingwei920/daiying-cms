...
<main class="wrap layout layout-<?= $context->e($layout) ?>">
    <section class="post-list<?= $listLayout === 'grid' ? ' post-list-grid' : '' ?>" aria-label="文章列表">
        <?php if ($items === []): ?>
            <article class="empty"><h2>这里还没有内容</h2><p><?= $context->e($emptyMessage) ?></p><p><a href="<?= $context->e(dy_site_url('/')) ?>">返回首页</a></p></article>
        <?php endif; ?>
    ...
    <?php if (dy_bool($context, 'sidebar_enabled', true)): ?>
        <aside class="sidebar" aria-label="侧栏">
            ...
            <section class="side-block"><h2>站点</h2><ul><li><a href="<?= $context->e(dy_site_url('/')) ?>">返回首页</a></li><li><a href="<?= $context->e(dy_site_url('/search')) ?>">搜索内容</a></li><li><a href="<?= $context->e(dy_site_url('/sitemap.xml')) ?>">站点地图</a></li></ul></section>
        </aside>
    <?php endif; ?>
</main>
