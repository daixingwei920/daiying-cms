...
final class UpdateService
{
    ...
    private function preflightRelease(string $releaseDir): void
    {
        ...
        $cmd = PHP_BINARY . ' -r ' . escapeshellarg('require "' . $releaseDir . '/system/core/Bootstrap/autoload.php"; echo "ok";');
        $output = [];
        $code = 0;
        exec($cmd, $output, $code);
        if ($code !== 0 || implode('', $output) !== 'ok') {
    ...
    {
        $cmd = PHP_BINARY . ' -l ' . escapeshellarg($file) . ' 2>&1';
        $output = [];
        $code = 0;
        exec($cmd, $output, $code);
        if ($code !== 0) {
            throw new UpdateException('PHP syntax preflight failed.');
        }
    }
