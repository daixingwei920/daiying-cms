...
final class PaidDownloadController
{
    ...
    public function checkout(Request $request): Response
    {
        if ($request->method !== 'POST') {
            return Response::text('付费下载购买请求必须通过 POST 提交。', 405)
        ...
        [$contentId, $mediaId] = $this->pathIds($request->path);
        try {
            ...
            $selector = new PaymentProviderSelector($pdo, $this->settings);
            $postedProviderId = $this->bodyString($request, 'provider_id');
            $providerId = $postedProviderId !== '' ? $selector->requireEnabled($postedProviderId) : $selector->defaultProviderId();
            $result = (new PaidDownloadService($pdo, $this->settings))->checkout(
                $contentId,
                $mediaId,
                $providerId,
                'paid-download-' . $contentId . '-' . $mediaId . '-' . bin2hex(random_bytes(8)),
            );
    ...
    public function complete(Request $request): Response
    {
        if ($request->method !== 'GET') {
            return Response::text('付费下载完成页只能通过 GET 访问。', 405)
                ->withHeaders(['Allow' => 'GET', 'Cache-Control' => 'private, no-store']);
        }

        [$contentId, $mediaId] = $this->pathIds($request->path, 'complete');
        try {
            $pdo = ConnectionFactory::make($this->settings);
            $result = (new PaidDownloadService($pdo, $this->settings))->completeHostedCheckout(
                $contentId,
                $mediaId,
    ...
    private function pathIds(string $path, string $action = 'checkout'): array
    {
        if (preg_match('#^/paid-download/([1-9][0-9]{0,17})/([1-9][0-9]{0,17})/' . preg_quote($action, '#') . '$#', $path, $matches) !== 1) {
            return [0, 0];
        }

        return [$this->canonicalPathId((string) $matches[1]), $this->canonicalPathId((string) $matches[2])];
    }
    ...
    {
        return Response::text('付费下载暂不可用，请稍后重试或联系网站管理员。', 500)
            ->withHeaders(['Cache-Control' => 'private, no-store']);
    }
    ...
    private function pendingResponse(array $result, string $title): Response
    {
        ...
        $downloadUrl = $this->safePendingPath((string) ($result['download_url'] ?? ''));
        ...
        $html = '<h1>等待支付确认</h1><p>' . View::escape($title) . '已经创建支付记录，管理员确认到账后会解锁访问。</p>' .
            ...
            '<p class="muted">此页面不会发放下载 Token；支付确认前文件仍由 CMS Core 锁定。</p>' .
            ($completionUrl !== '' ? '<p><a class="button" href="' . View::escape($completionUrl) . '">检查确认状态</a></p>' : '') .
            '<p><a class="button" href="' . View::escape($downloadUrl !== '' ? $downloadUrl : '/') . '">返回内容</a></p>';
