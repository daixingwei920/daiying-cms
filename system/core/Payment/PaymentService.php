...
final class PaymentService
{
    ...
    public function enabledProviders(): array
    {
        ...
        foreach ($repo->all() as $setting) {
            ...
            if (!$this->providerCheckoutAvailable($providerId, $setting)) {
                continue;
            }
    ...
    public function createProviderPayment(
        ...
        try {
            ...
            $this->assertSafeProviderRedirectData($result->data);
            ...
            $transientCheckoutUrl = $this->providerCheckoutUrlFromResultData($result->data);
    ...
    private function providerCheckoutAvailable(string $providerId, array $setting): bool
    {
        $provider = PaymentProviderRegistry::get($providerId);
        if ($provider === null || !in_array('payment.create', $provider->capabilities(), true)) {
            return false;
    ...
    /** @param array<string,mixed> $data */
    private function assertSafeProviderRedirectData(array $data): void
    {
        foreach (['checkout_url', 'payment_url', 'redirect_url'] as $key) {
            if (!array_key_exists($key, $data) || $data[$key] === null || $data[$key] === '') {
                continue;
            }
            if (!is_string($data[$key]) || !$this->isSafeProviderRedirectUrl($data[$key])) {
    ...
    /** @param array<string,mixed> $data */
    private function providerCheckoutUrlFromResultData(array $data): string
    {
        foreach (['checkout_url', 'payment_url', 'redirect_url'] as $key) {
            $url = $data[$key] ?? '';
            if (is_string($url) && $url !== '' && $this->isSafeProviderRedirectUrl($url)) {
                return $url;
            }
        }
