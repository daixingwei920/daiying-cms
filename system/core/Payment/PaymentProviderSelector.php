...
final class PaymentProviderSelector
{
    ...
    public function defaultProviderId(): string
    {
        ...
        foreach ($this->enabledProviderSettings() as $setting) {
            ...
            if (
                ...
                && $this->checkoutAvailable($providerId, $setting)
            ) {
        ...
        foreach ($this->enabledProviderSettings() as $setting) {
            $providerId = (string) ($setting['provider_id'] ?? '');
            if ($this->checkoutAvailable($providerId, $setting)) {
                return $providerId;
            }
        }
    ...
    public function requireEnabled(string $providerId): string
    {
        try {
        ...
        if ($setting === null || !$this->checkoutAvailable($providerId, $setting)) {
    ...
    public function enabledProviders(): array
    {
        ...
        foreach ($this->enabledProviderIds() as $providerId) {
            ...
            if ($provider === null || !is_array($setting) || !$this->checkoutAvailable($providerId, $setting)) {
                continue;
            }
    ...
    private function checkoutAvailable(string $providerId, array $setting): bool
    {
        if (!$this->supportsCheckout($providerId)) {
            return false;
        }
