...
final class LocalPluginPackageInstaller
{
    ...
    public function preview(string $zipPath, int $adminId): array
    {
        ...
        try {
            ...
            if ($source === 'official') {
                throw new PluginException('Official signed packages must use the official market verification flow.');
            }
    ...
    private function readManifest(string $staging): array
    {
        ...
        if (($decoded['package_type'] ?? 'plugin') !== 'plugin') {
            throw new PluginException('Package type must be plugin.');
        }
