...
final class CardDeliveryService
{
    ...
    public function checkout(int $productId, string $providerId, string $idempotencyKey = '', int $quantity = 1): array
    {
        ...
        $providerId = $providerId !== ''
            ? (new PaymentProviderSelector($this->pdo, $settings))->requireEnabled($providerId)
            : (new PaymentProviderSelector($this->pdo, $settings))->defaultProviderId();
    ...
    private function providerCheckoutUrl(array $payment): string
    {
        ...
        if (is_string($transientUrl) && $this->isSafeProviderRedirectUrl($transientUrl)) {
            return $transientUrl;
        }
        $metadata = $this->paymentMetadata($payment);
        foreach (['checkout_url', 'payment_url', 'redirect_url'] as $key) {
            $url = $metadata[$key] ?? '';
            if (is_string($url) && $this->isSafeProviderRedirectUrl($url)) {
                return $url;
            }
        }
