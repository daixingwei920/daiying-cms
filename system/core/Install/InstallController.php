...
final class InstallController
{
    ...
    public function store(Request $request): Response
    {
        ...
        $siteId = $siteId !== '' ? $siteId : 'site-' . bin2hex(random_bytes(8));
    ...
    private function form(string $error = '', string $success = '', ?Request $request = null): string
    {
        ...
        $siteId = $this->value($request, 'site_id', 'site-' . bin2hex(random_bytes(8)));
        $siteSecret = $this->value($request, 'site_secret', bin2hex(random_bytes(32)));
        ...
        return '<h1>安装 PHP CMS</h1><p class="muted">安装向导会完成环境检查、数据库连接测试、站点身份、管理员、Core migrations、默认主题和安装锁。</p>' .
            ...
            '<h2>5. 站点身份/密钥</h2>' .
            '<label>站点 ID<input name="site_id" value="' . View::escape($siteId) . '" required></label>' .
            '<label>站点密钥<input name="site_secret" value="' . View::escape($siteSecret) . '" required></label>' .
            '<p class="muted">安装成功后会写入配置文件和安装锁，普通 URL 不会再次触发重装。</p>' .
    ...
    private function writeConfig(array $database, string $siteName, string $siteUrl, string $siteId, string $siteSecret): void
    {
        ...
        $items['security']['admin_mfa']['implemented_methods'] = ['totp', 'recovery_codes'];
