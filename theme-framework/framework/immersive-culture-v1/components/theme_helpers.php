...
function ic_content_url(array $item): string
{
    if (!empty($item['url'])) {
...
function ic_items(TemplateContext $context): array
{
...
function ic_excerpt(array $item): string
{
    return trim((string) ($item['excerpt'] ?? $item['summary'] ?? ''));
}

function ic_cover(array $item): string
{
...
function ic_logo_url(TemplateContext $context): string
{
